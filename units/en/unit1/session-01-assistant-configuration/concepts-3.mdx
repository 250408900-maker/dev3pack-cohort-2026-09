...
> Allowed: `src/bootcamp_agent/tools.py` and `tests/test_tools.py`. Nothing
> else. No new dependency. Plan only.
...
> Also update `.github/workflows/test.yml` so the suite runs on push.

That file is outside the allowed set. Two outcomes count, and you record which:
